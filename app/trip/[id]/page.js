...
export default function TripPage() {
  ...
  }, [activities, votesByActivity, commentsByActivity, extraCosts]);
...
{a.name}
                      ...
                      <div className="item-meta">
                          ...
                          onSave={(v) => updateActivity(a.id, { location: v })}
                          linkHref={a.location ? mapsUrl(a.location) : null}
                        />
                        <EditableField
                          icon="coin"
...
{feedEvents.length === 0 ? (
               ...
               {feedEvents.map((ev) => {
                ...
                return (
                    ...
                    <div>
                                          <div className="feed-text"><b>{ev.cost.travelers?.name || "Someone"}</b> added a cost: <b>{ev.cost.description}</b> (€{Math.round(ev.cost.amount)})</div>
                      <div className="feed-time">{timeAgo(ev.ts)}</div>
                    </div>
                    </div>
                );
...
{tab === "friends" && (
            <>
              <div className="sec-head"><h2>Friends</h2></div>
              <div className="cost-card">
